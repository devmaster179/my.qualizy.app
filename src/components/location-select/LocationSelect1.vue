  ...
  <div class="vx-auto-suggest">
    ...
    >
      <li ref="option" class="auto-suggest__suggestion cursor-pointer">
        <VuePerfectScrollbar class="location-select-scroll-area" :settings="settings">
          <div class="flex items-center justify-between location-items py-2 px-2">
            <vs-checkbox
              v-model="selectAllLocations"
              icon-pack="feather"
              :icon="selectAllIcon"
              class="karla text-lg"
            >{{locations.key}} ({{selectedLocations.length}}/{{locations.count}})</vs-checkbox>
          </div>
          <ul class="overflow-hidden">
            ...
            >
              ...
              >
                <div class="flex items-center" @click.stop="chnCountryValues(country.key)">
                  <vs-icon
                  ...
                  />
                  <span
                    class="karla-bold ml-1"
                  >{{country.key}} ({{countryList(country.key).ids.length}}/{{countryList(country.key).cnt.length}})</span>
                </div>
                <vs-icon
                  icon-pack="feather"
              ...
              <ul class="overflow-hidden" :class="{'hidden':openCountries.indexOf(country.key)<0}">
                <li
                  class="cursor-pointer"
                  ...
                  <template v-else>
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          class="karla-bold ml-1"
                        >{{region.key}} ({{regionList(country.key , region.key).ids.length}}/{{regionList(country.key , region.key).cnt.length}})</span>
                      </div>
                      <vs-icon
                        icon-pack="feather"
                        icon="icon-chevron-down"
                      ...
                      >
                        ...
                        <template v-else>
                          ...
                          >
                            <div
                              class="flex items-center ml-1"
                              @click.stop="chnAreaValues(country.key , region.key , area.key)"
                            >
                              <vs-icon
                                :icon="areaList(country.key , region.key , area.key).icon"
                                :color="areaList(country.key , region.key , area.key).color"
                                size="19px"
                              />
                              <span
                                class="karla-bold ml-1"
                              >{{area.key}} ({{areaList(country.key , region.key , area.key).ids.length}}/{{areaList(country.key , region.key , area.key).cnt.length}})</span>
                            </div>
