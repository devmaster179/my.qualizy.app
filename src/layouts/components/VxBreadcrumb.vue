...
<template functional>
  <div class="vx-breadcrumb" :class="data.staticClass">
    <ul class="flex flex-wrap items-center">
      ...
      >
        <router-link :to="link.url" v-if="link.url">{{ link.title | capitalize}}</router-link>
        <span class="text-primary cursor-default" v-else>{{ link.title | capitalize}}</span>
      ...
      <li class="inline-flex">
        ...
        >{{(props.route.meta.breadcrumb.slice(-1)[0].title) | capitalize }}</span>
      </li>
    </ul>
  </div>
</template>
