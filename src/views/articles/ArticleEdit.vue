<template>
  <vs-popup :title="$t('update article') + `(${article.title})`" :active.sync="activeOpen">
    <div class="p-4">
      ...
      <div class="article-actions flex justify-end mt-base">
        ...
        <vs-button
          ...
          style="padding:.8rem 2rem!important;"
        >
          <span class="karla ml-2">{{$t("update")}}</span>
        </vs-button>
      </div>
    </div>
  </vs-popup>
...
export default {
  ...
  methods: {
      ...
      if (this.file.length > 0) {
        ...
        if (this.file.length > 0) {
          ...
          if (size > 1) {
            this.$vs.notify({
              title: "Error",
              text: "Your file is too big! Please select an image under 1MB",
              iconPack: "feather",
            ...
            storageRef.child(this.article.ref).delete();
            let ref =
                ...
                docUpload.snapshot.ref.getDownloadURL().then((downloadURL) => {
                  ...
                  db.collection("knowledge")
                    ...
                    .update({
                      ...
                      updated_by: JSON.parse(localStorage.getItem("userInfo"))
                        .id,
                      updated_at: new Date(),
                    });
                  this.$vs.notify({
                    title: "Success",
                    text: "A article is updated successfully",
                    iconPack: "feather",
                    icon: "icon-alert-circle",
                    color: "success",
                  });
    ...
    handleDrop(e) {
      ...
      if (!this.isPDF(rawFile) && !this.isVideo(rawFile[0])) {
        this.$vs.notify({
          title: "Error",
          text: "Only supports upload .pdf or video suffix files",
          iconPack: "feather",
