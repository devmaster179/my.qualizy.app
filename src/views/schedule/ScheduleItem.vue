<template>
  ...
  >
    <div class="vx-row">
      <div class="vx-col w-full sm:w-10/12 flex sm:items-center sm:flex-row flex-col flex-wrap">
        <div class="flex items-center">
          <vs-avatar
            class="sender__avatar flex-shrink-0 mr-3 border-2 border-solid border-white"
            :src="require(`@/assets/images/template_image/${templateInfo(schedule.template).content.templateImage}`)"
            size="40px"
          ></vs-avatar>
          <h5
            class="mb-1 mr-4 templateTitle"
          >{{ templateInfo(schedule.template).content.templateTitle }} - {{$t(schedule._repeat)}}</h5>
        </div>
        <div class="todo-tags sm:ml-2 sm:my-0 my-2 flex flex-wrap">
          <vs-chip v-for="(user, index) in schedule.assign" :key="index">
            <div
              ...
              :class="userInfo(user)!==undefined && userInfo(user).active ? 'bg-success' : 'bg-warning'"
            ></div>
            <span style v-if="userInfo(user)!==undefined">{{ userInfo(user).name | capitalize }}</span>
          </vs-chip>
        </div>
      </div>
      <div class="vx-col w-full sm:w-2/12 ml-auto flex justify-end text-right">
        <vs-switch @click.stop="chnActive" :disabled="role>2" v-model="active" class="ml-4 mt-1" />
        <vs-icon
          icon-pack="feather"
          v-if="role<2"
          icon="icon-trash-2"
