  ...
  <div id="schdules-page" class="px-2">
    ...
    <template v-else>
      <div class="page-header flex items-center justify-between">
        <p class="page-title karla-bold color-my-black text-2xl">
          {{ $t("schedule") }}
        </p>
        ...
        <template v-else>
          <div class="flex w-full mt-base">
            ...
            >
              ...
              >
                {{ $t("You don’t have any tasks scheduled yet") }},
                {{ $t("would you like to create one") }}?
              </h5>
              <vs-button v-if="role < 2" class="ml-2" @click="newSchedule">{{
                $t("new schedule")
              }}</vs-button>
            </div>
          </div>
    ...
    schedules() {
      ...
      schedules.map((schedule) => {
        ...
        if (locationList.length > 0) {
          ...
          if (!schedule.location.some((sl) => locationList.includes(sl)))
            return;
        }
        ...
        if (
          template === undefined ||
          template.content.templateSD == "bookmarked"
        )
          return;
        // if (locationList.length > 0) {
