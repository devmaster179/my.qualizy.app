    ...
    <div slot="no-body">
      <div
        v-if="
          role > 1 &&
          task.time !== undefined &&
          calcTime(task.time) > 0 &&
          calcTime(task.time) < 7200 * 1000
        "
      ...
      >
        ...
        <span
          ...
          style="font-size: 12px; color: rgba(var(--vs-danger), 0.8)"
          >{{
            calcTime(task.time) | duration("humanize", true) | leftTime
          }}
          left</span
        >
      </div>
      <div class="pt-8 pb-4 px-5">
        <div class="flex items-center mb-5">
          ...
          <div style="width: calc(100% - 4rem)">
            <p class="karla-bold templateTitle truncate">
        ...
        <div class="flex justify-between">
          <p class="karla-bold dueText" v-if="task.time !== undefined">
            ...
            <label v-if="progress" class="dueText text-primary">{{
              $t("in progress")
            }}</label>
            <label
              v-else
            ...
            >
            <span class="pl-2"
              >{{ calcTime(task.time) < 0 ? "" : "Due" }}
              {{
                calcTime(task.time) | duration("humanize", true) | capitalize
              }}</span
            >
          </p>
          <p
        ...
        ></vs-progress>
        <div
          v-for="(label, index) in templateLabels(task.templateID)"
          :key="index"
          class="p-2 rounded-lg ml-1 templateLabelBadge inline-block mt-2"
          :class="[
            {
              hidden: label == undefined || (index != 0 && !labelColapes),
            },
          ]"
        >
          <template v-if="label != undefined">
            <div class="flex items-center">
              ...
              ></div>
              <p class="karla">{{ label.name | truncate(20) }}</p>
            </div>
          </template>
        </div>
        <div
          v-if="templateLabels(task.templateID).length > 1 && !labelColapes"
          class="p-2 rounded-lg ml-1 templateLabelBadge inline-block mt-2"
        >
          <span class="karla-bold"
            >+ {{ templateLabels(task.templateID).length - 1 }}</span
          >
        </div>
        <div
          v-if="templateLabels(task.templateID).length > 1"
        ...
        <div
          ...
          v-if="task.logs !== undefined"
        >
          <div></div>
          <div class="flex items-center">
            <vs-icon icon-pack="feather" icon="icon-clock"></vs-icon>
            <span class="karla ml-2 locationText">{{
        ...
        <div class="flex items-center justify-between mt-2">
          <div>
            <vs-icon
              @click.stop="deleteLog"
              v-if="
                progress || (task.logs !== undefined && task.time === undefined)
              "
...
export default {
  ...
  computed: {
      ...
      return (id) => {
        ...
        if (
          ...
          locations = locations.filter(
            (item) => !!this.$store.getters["app/getLocationById"](item)
          );
          if (locations.length == 0) return this.$t("no location");
          else {
    ...
    templateLabels() {
      return (id) => {
        let labels = this.$store.getters["app/getTemplateById"](id).content
          .templateLabel;

        let filtered = [];
        labels.map((label_id) => {
          let label = this.$store.getters["app/getLabelById"](label_id);
          if (label == undefined) {
            return false;
          } else if (label.group == "global") {
            return false;
          }
          filtered.push(label);
          return true;
