<template>
  <div class="report-content px-base" v-if="Object.keys(this.item).length > 0">
    <table class="w-full" style=" border-spacing: 0px;">
      ...
      <tbody>
        <tr v-for="(logTR,index) in logData" :key="index" style=" border-spacing: 0px;">
          <template v-if="logTR.field=='template'">
            <td>
              <div class="mt-base flex flex-wrap justify-between items-center w-full">
                <div class="flex items-center">
                  ...
                  <div class="flex flex-col">
                    <h5
                      class="mb-1"
                    >{{getTemplateInfo(logTR.data).content.templateTitle | capitalize}}</h5>
                  </div>
                </div>

                <div class="flex-col" style="margin-left:auto;">
                  <div class="flex self-end whitespace-no-wrap" style="width:150px;">
                    <vs-progress :percent="complated" :color="calcColor()" />
                  </div>
                  <div class="flex sm:mr-0 mr-2 self-end whitespace-no-wrap">{{calcComplate()}}</div>
                  <div class="mt-2">
                    <span
                      class="text-success"
                    >{{item.updated_at.toDate()|moment('DD/MMM/YYYY @ HH[h]mm')}}</span>
                  </div>
                </div>
              </div>
            </td>
          ...
          <template v-else-if="logTR.field=='question'">
            <table class="w-full px-base mb-4 questionTable" style=" border-spacing: 0px;">
              <thead>
                <tr>
                  <th></th>
                </tr>
              ...
              <tbody>
                <tr v-for="(questionTR , qIndex) in logTR.data" :key="index + '_' + qIndex ">
                  <template v-if="questionTR.field == 'question'">
                    <td colspan="3">
                      <h6 class="bg-success p-3 text-white">{{questionTR.title}}</h6>
                    </td>
                  </template>
                  <!-- <template
                  ...
                  <template v-if="questionTR.field == 'answer'">
                    ...
                    >
                      ...
                      <span v-if="questionTR.data.ref.mandatory" class="text-danger">*</span>
                    </td>
                    <td class="p-2 border border-solid d-theme-border-grey-light border-t-0">
                      ...
                      >
                        ...
                        >{{questionTR.data.value | moment('DD MMMM ,YYYY')}}</span>
                        <span
                          v-else-if="questionTR.data.ref.type.dateType=='Time'"
                        >{{questionTR.data.value | moment('HH:mm')}}</span>
                        <span
                          v-else-if="questionTR.data.ref.type.dateType=='Date & Time'"
                        >{{questionTR.data.value | moment('DD MMMM ,YYYY @ HH[h]mm')}}</span>
                      </template>
                      <span v-else-if="getType(questionTR.data.ref.type.id).content == 'star'">
                        <star-rating
                          :star-size="25"
                          :read-only="true"
                          :increment="0.1"
                      ...
                      <span
                        v-else-if="getType(questionTR.data.ref.type.id).content == 'automatic user stamp'"
                      >{{getUserInfo(questionTR.data.value).name | capitalize}}</span>
                      <div
                        v-else-if="getType(questionTR.data.ref.type.id).content=='short answer' || getType(questionTR.data.ref.type.id).content=='paragraph'"
                        class="text-primary"
                        style="font-style: italic; white-space: pre-wrap;"
                      >{{questionTR.data.value}}</div>
                      <vs-checkbox
                        v-else-if="getType(questionTR.data.ref.type.id).content=='checkbox'"
                        disabled="true"
                        :value="questionTR.data.value"
                      ></vs-checkbox>
                      <template v-else-if="getType(questionTR.data.ref.type.id).content=='items'">
                        <span
                          v-if="getItemInfo(questionTR.data.value)!=undefined"
                        >{{getItemInfo(questionTR.data.value).name}}</span>
                      </template>
                      <template v-else-if="getType(questionTR.data.ref.type.id).content=='users'">
                        <span
                          v-if="getUserInfo(questionTR.data.value)!=undefined"
                        >{{getUserInfo(questionTR.data.value).name | capitalize}}</span>
                      </template>
                      <template v-else-if="getType(questionTR.data.ref.type.id).content=='teams'">
                        <span
                          v-if="getTeamInfo(questionTR.data.value)!=undefined"
                        >{{getTeamInfo(questionTR.data.value).name | capitalize}}</span>
                      </template>
                      <template
                      ...
                      <div
                        v-else-if="getType(questionTR.data.ref.type.id).content=='instruction'"
                        class="text-primary"
                        style="font-style: italic; white-space: pre-wrap;"
                      >{{questionTR.data.ref.type.instruction}}</div>
                      <template
                        v-else-if="getType(questionTR.data.ref.type.id).content=='signature'"
                      >
                        <img
                          :src="questionTR.data.value.url"
                          v-if="questionTR.data.value.url!==undefined && questionTR.data.value.url!=''"
                        />
                      </template>
                      <template
                        v-else-if="getType(questionTR.data.ref.type.id).type=='closed answers'"
                      >
                        <span v-if="getType(questionTR.data.ref.type.id).group=='global'" :class="[{'text-danger':questionTR.data.ref.type.failedAnswer == questionTR.data.value }]">{{$t(questionTR.data.value) | capitalize}}</span>
                        <span v-else :class="[{'text-danger':questionTR.data.ref.type.failedAnswer == questionTR.data.value }]">{{questionTR.data.value | capitalize}}</span>
                      </template>
                      <span
                        v-else
                        
                      >{{questionTR.data.value}}</span>
                    </td>
                    <!-- <td
